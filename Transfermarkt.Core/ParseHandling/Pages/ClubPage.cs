...
namespace Transfermarkt.Core.ParseHandling.Pages
{
    ...
    class ClubPlayersPageSection : ChildsSamePageSection<Player, IValue, HtmlNode>
    {
        public ClubPlayersPageSection(HAPConnection connection, ILogger logger)
        {
            ...
            this.Parsers.ToList().ForEach(p => p.OnSuccess += (o, e) => logger.LogMessage(LogLevel.Info, new List<string> { $"Success parsing {e.Element.name}." }));
            this.Parsers.ToList().ForEach(p => p.OnFailure += (o, e) => logger.LogException(LogLevel.Warning, new List<string> { $"Error parsing {e.Element.name} on node {e.Node?.Name}.", $"INNER_TEXT: {e.Node?.InnerText}", $"INNER_HTML: {e.Node?.InnerHtml}" }, e.Exception));
        }
    }
}
